...
def test_valid_billiecoin_address():
    from billiecoinlib import is_valid_billiecoin_address

    main = valid_billiecoin_address()
    test = valid_billiecoin_address('testnet')
    main_btc = valid_bitcoin_address()
    test_btc = valid_bitcoin_address('testnet')

    assert is_valid_billiecoin_address(main) is True
    assert is_valid_billiecoin_address(main, 'mainnet') is True
    assert is_valid_billiecoin_address(main, 'testnet') is False

    assert is_valid_billiecoin_address(test) is False
    assert is_valid_billiecoin_address(test, 'mainnet') is False
    assert is_valid_billiecoin_address(test, 'testnet') is True

    assert is_valid_billiecoin_address(main_btc) is True
    assert is_valid_billiecoin_address(main_btc, 'mainnet') is True
    assert is_valid_billiecoin_address(main_btc, 'testnet') is False

    assert is_valid_billiecoin_address(test_btc) is False
    assert is_valid_billiecoin_address(test_btc, 'mainnet') is False
    assert is_valid_billiecoin_address(test_btc, 'testnet') is True


def test_invalid_billiecoin_address():
    from billiecoinlib import is_valid_billiecoin_address

    main = invalid_billiecoin_address()
    test = invalid_billiecoin_address('testnet')
    main_btc = invalid_bitcoin_address()
    test_btc = invalid_bitcoin_address('testnet')

    assert is_valid_billiecoin_address(main) is False
    assert is_valid_billiecoin_address(main, 'mainnet') is False
    assert is_valid_billiecoin_address(main, 'testnet') is False

    assert is_valid_billiecoin_address(test) is False
    assert is_valid_billiecoin_address(test, 'mainnet') is False
    assert is_valid_billiecoin_address(test, 'testnet') is False

    assert is_valid_billiecoin_address(main_btc) is False
    assert is_valid_billiecoin_address(main_btc, 'mainnet') is False
    assert is_valid_billiecoin_address(main_btc, 'testnet') is False

    assert is_valid_billiecoin_address(test_btc) is False
    assert is_valid_billiecoin_address(test_btc, 'mainnet') is False
    assert is_valid_billiecoin_address(test_btc, 'testnet') is False
...
def test_deterministic_masternode_elections(current_block_hash, mn_list):
    from billiecoinlib import elect_mn
...
def test_parse_masternode_status_vin():
    from billiecoinlib import parse_masternode_status_vin
...
def test_hash_function():
    ...
    hex_hash = "%x" % billiecoinlib.hashit(sb_data_hex)
...
def test_blocks_to_seconds():
    ...
    assert Decimal(billiecoinlib.blocks_to_seconds(0)) == Decimal(0.0)
    assert Decimal(billiecoinlib.blocks_to_seconds(2)).quantize(precision) \
        == Decimal(120).quantize(precision)
    assert int(billiecoinlib.blocks_to_seconds(16616)) == 996960
