...
sentinel_cfg = BilliecoinConfig.tokenize(sentinel_config_file)
sentinel_version = "1.1.0"
min_billiecoind_proto_version_with_sentinel_ping = 70207
...
def get_billiecoin_conf():
    args = get_args()

    if args.config:
        billiecoin_conf = args.config
    else:
        home = os.environ.get('HOME')
        if home is not None:
            if sys.platform == 'darwin':
                billiecoin_conf = os.path.join(home, "Library/Application Support/BilliecoinCore/billiecoin.conf")
            else:
                billiecoin_conf = os.path.join(home, ".billiecoincore/billiecoin.conf")
        else:
            home = os.getenv('APPDATA')
            if home is not None:
                billiecoin_conf = os.path.join(home, "billiecoincore\\billiecoin.conf")
            else:
                billiecoin_conf = 'billiecoin.conf'
        
        billiecoin_conf = sentinel_cfg.get('billiecoin_conf', billiecoin_conf)

    return billiecoin_conf
...
billiecoin_conf = get_billiecoin_conf()
network = get_network()
db = get_db_conn()
