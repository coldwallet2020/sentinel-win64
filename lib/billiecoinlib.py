...
def is_valid_billiecoin_address(address, network='mainnet'):
    ...
    try:
        ...
        # rescue from exception, not a valid Billiecoin address
        return False

    if (address_version != billiecoin_version and address_version != bitcoin_version):
        return False
...
def SHIM_serialise_for_billiecoind(sentinel_hex):
    from models import GOVOBJ_TYPE_STRINGS

    # unpack
    obj = deserialise(sentinel_hex)

    # shim for billiecoind
    govtype_string = GOVOBJ_TYPE_STRINGS[obj['type']]

    # superblock => "trigger" in billiecoind
    if govtype_string == 'superblock':
        govtype_string = 'trigger'

    # billiecoind expects an array (will be deprecated)
    obj = [(govtype_string, obj,)]

    # re-pack
    billiecoind_hex = serialise(obj)
    return billiecoind_hex
...
def did_we_vote(output):
    ...
    try:
        detail = output.get('detail').get('billiecoin.conf')
