...
class BilliecoinDaemon():
    def __init__(self, **kwargs):
        ...
        # memoize calls to some billiecoind methods
    ...
    @classmethod
    def from_billiecoin_conf(self, billiecoin_dot_conf):
        from billiecoin_config import BilliecoinConfig
        config_text = BilliecoinConfig.slurp_config_file(billiecoin_dot_conf)
        creds = BilliecoinConfig.get_rpc_creds(config_text, config.network)
    ...
    def get_my_gobject_votes(self, object_hash):
        ...
        if not self.gobject_votes.get(object_hash):
            ...
            self.gobject_votes[object_hash] = billiecoinlib.parse_raw_votes(raw_votes)

        return self.gobject_votes[object_hash]

    def is_govobj_maturity_phase(self):
        # 3-day period for govobj maturity
        maturity_phase_delta = 4320      # ~(60*24*3)/2.6
    ...
    def we_are_the_winner(self):
        ...
        winner = billiecoinlib.elect_mn(block_hash=current_block_hash, mnlist=mn_list)
        my_vin = self.get_current_masternode_vin()
    ...
    def estimate_block_time(self, height):
        ...
        future_seconds = billiecoinlib.blocks_to_seconds(diff)
    ...
    def has_sentinel_ping(self):
        ...
        return (getinfo['protocolversion'] >= config.min_billiecoind_proto_version_with_sentinel_ping)
