...
class GovernanceObject(BaseModel):
    ...
    @classmethod
    def sync(self, billiecoind):
        golist = billiecoind.rpc_command('gobject', 'list')
        ...
        try:
            ...
            for item in golist.values():
                (go, subobj) = self.import_gobject_from_billiecoind(billiecoind, item)
    ...
    @classmethod
    def import_gobject_from_billiecoind(self, billiecoind, rec):
        import decimal
        ...
        # shim/billiecoind conversion
        object_hex = billiecoinlib.SHIM_deserialise_from_billiecoind(object_hex)
        objects = billiecoinlib.deserialise(object_hex)
        subobj = None
        ...
        # exclude any invalid model data from billiecoind...
        valid_keys = subclass.serialisable_fields()
        subdikt = {k: dikt[k] for k in valid_keys if k in dikt}

        # get/create, then sync vote counts from billiecoind, with every run
        ...
        # get/create, then sync payment amounts, etc. from billiecoind - Billiecoind is the master
        try:
            newdikt = subdikt.copy()
            newdikt['object_hash'] = object_hash
            if subclass(**newdikt).is_valid() is False:
                govobj.vote_delete(billiecoind)
                return (govobj, None)

            subobj, created = subclass.get_or_create(object_hash=object_hash, defaults=subdikt)
        except Exception as e:
            # in this case, vote as delete, and log the vote in the DB
            printdbg("Got invalid object from billiecoind! %s" % e)
            govobj.vote_delete(billiecoind)
            return (govobj, None)
    ...
    def vote_delete(self, billiecoind):
        if not self.voted_on(signal=VoteSignals.delete, outcome=VoteOutcomes.yes):
            self.vote(billiecoind, VoteSignals.delete, VoteOutcomes.yes)
        return
    ...
    def vote(self, billiecoind, signal, outcome):
        import billiecoinlib
        ...
        output = billiecoind.rpc_command(*vote_command)

        # extract vote output parsing to external lib
        voted = billiecoinlib.did_we_vote(output)
        ...
        if voted:
            ...
            self.sync_network_vote(billiecoind, signal)

    def sync_network_vote(self, billiecoind, signal):
        printdbg('\tsyncing network vote for object %s with signal %s' % (self.object_hash, signal.name))
        vote_info = billiecoind.get_my_gobject_votes(self.object_hash)
...
class Proposal(GovernanceClass, BaseModel):
    ...
    govobj_type = BILCOD_GOVOBJ_TYPES['proposal']
    ...
    def is_valid(self):
        ...
        try:
            ...
            # payment address is valid base58 billiecoin addr, non-multisig
            if not billiecoinlib.is_valid_billiecoin_address(self.payment_address, config.network):
                printdbg("\tPayment address [%s] not a valid Billiecoin address for network [%s], returning False" % (self.payment_address, config.network))
                return False
    ...
    def is_expired(self, superblockcycle=None):
        ...
        expiration_window_seconds = int(
            (billiecoinlib.blocks_to_seconds(superblockcycle) / 2) +
    ...
    def is_deletable(self):
        ...
        # TBD (item moved to external storage/BilliecoinDrive, etc.)
        return False
    ...
    def get_prepare_command(self):
        import billiecoinlib
        obj_data = billiecoinlib.SHIM_serialise_for_billiecoind(self.serialise())
    ...
    def prepare(self, billiecoind):
        try:
            object_hash = billiecoind.rpc_command(*self.get_prepare_command())
...
class Superblock(BaseModel, GovernanceClass):
    ...
    govobj_type = BILCOD_GOVOBJ_TYPES['superblock']
    ...
    def is_valid(self):
        ...
        for addr in addresses:
            if not billiecoinlib.is_valid_billiecoin_address(addr, config.network):
    ...
    def hash(self):
        import billiecoinlib
        return billiecoinlib.hashit(self.serialise())
...
class Watchdog(BaseModel, GovernanceClass):
    ...
    govobj_type = BILCOD_GOVOBJ_TYPES['watchdog']
    only_masternode_can_submit = True

    @classmethod
    def active(self, billiecoind):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at >= (now - billiecoind.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
        return resultset

    @classmethod
    def expired(self, billiecoind):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at < (now - billiecoind.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
    ...
    def is_expired(self, billiecoind):
        now = int(time.time())
        return (self.created_at < (now - billiecoind.SENTINEL_WATCHDOG_MAX_SECONDS))

    def is_valid(self, billiecoind):
        if self.is_expired(billiecoind):
            return False

        return True

    def is_deletable(self, billiecoind):
        if self.is_expired(billiecoind):
            return True
