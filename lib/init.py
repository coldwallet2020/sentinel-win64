...
def has_billiecoin_conf():
    import config
    import io

    valid_billiecoin_conf = False

    # ensure billiecoin_conf exists & readable
    #
    # if not, print a message stating that Billiecoin Core must be installed and
    # configured, including JSONRPC access in billiecoin.conf
    try:
        f = io.open(config.billiecoin_conf)
        valid_billiecoin_conf = True
    except IOError as e:
        print(e)

    return valid_billiecoin_conf
...
def main():
    ...
    if not has_billiecoin_conf():
        print("BilliecoinCore must be installed and configured, including JSONRPC access in billiecoin.conf")
