...
class GovernanceClass(object):
    ...
    def vote(self, billiecoind, signal, outcome):
        return self.go.vote(billiecoind, signal, outcome)

    # pass thru to GovernanceObject#voted_on
    def voted_on(self, **kwargs):
        return self.go.voted_on(**kwargs)

    def vote_validity(self, billiecoind):
        if self.is_valid():
            printdbg("Voting valid! %s: %d" % (self.__class__.__name__, self.id))
            self.vote(billiecoind, models.VoteSignals.valid, models.VoteOutcomes.yes)
        else:
            printdbg("Voting INVALID! %s: %d" % (self.__class__.__name__, self.id))
            self.vote(billiecoind, models.VoteSignals.valid, models.VoteOutcomes.no)
    ...
    def list(self):
        ...
        # return a dict similar to billiecoind "gobject list" output
    ...
    def billiecoind_serialise(self):
        import billiecoinlib
        return billiecoinlib.SHIM_serialise_for_billiecoind(self.serialise())
